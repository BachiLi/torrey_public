...
\begin{equation}
\rho_{\text{phong-specular}} \propto K_s \max\left(r \cdot \omega, 0\right)^\alpha \max\left(n_s \cdot \omega, 0\right),
\end{equation}
...
\begin{equation}
\rho_{\text{phong-specular}} = K_s \frac{\alpha + 2}{2\pi} \max\left(r \cdot \omega, 0\right)^\alpha \max\left(n_s \cdot \omega, 0\right).
\end{equation}

Finally, we can do something similar to \lstinline{plastic} to combine a diffuse component and $\rho_{\text{phong-specular}}$ using Fresnel reflection:
\begin{equation}
\rho_{\text{phong}} = (1 - F) \frac{K_d}{\pi} \max\left(n_s \cdot \omega, 0\right) + F \rho_{\text{phong-specular}}.
\label{eq:phong}
\end{equation}
$F$ is defined the same as the Fresnel in \lstinline{plastic}, which depends on another parameter \lstinline{eta}. As long as $K_d \leq 1$ and $K_s \leq 1$, the energy is conserved.
