...
To apply inverse transform sampling, we note that $\phi$ can be handled separately and focus on $\theta$. We first get the normalization factor of $p_{\text{phong}}(\theta)$:
\begin{equation}
\begin{aligned}
& p_{\text{phong}}(\theta) \propto \left(\cos\theta\right)^{\alpha}\sin\theta \\
& \int_0^{\frac{\pi}{2}} \left(\cos\theta\right)^{\alpha}\sin\theta \mathrm{d}\theta = \frac{1}{\alpha + 1} \\
& p_{\text{phong}}(\theta) = \left(\alpha + 1\right) \left(\cos\theta\right)^{\alpha}.
\end{aligned}
\end{equation}
Next, we integrate to get the CDF:
\begin{equation}
M_{\theta}^{-1}(\theta) = \int_{0}^{\theta} p_{\text{phong}}(\hat{\theta})\mathrm{d}\hat{\theta} = \left.-\left(\cos\hat{\theta}\right)^{\alpha + 1}\right|_{\hat{\theta}=0}^{\theta} = 1 - \left(\cos\theta\right)^{\alpha + 1}
\end{equation}
Next, we invert to get the change of variable:
\begin{equation}
\theta = M_{\theta}(u_1) = \cos^{-1}\left(\left(1 - u_1\right)^{\frac{1}{\alpha + 1}}\right).
\label{eq:phong_theta}
\end{equation}
The mapping for $\phi$ is easy:
\begin{equation}
...
The Blinn-Phong BRDF fixes the two issues with one trick. Instead of defining a cosine lobe round the mirror reflection direction, we use the cosine between the normal and the half vector $h = \frac{\omega_i + \omega_o}{\left\|\omega_i + \omega_o\right\|}$:
\begin{equation}
\begin{aligned}
\rho_{\text{blinn}} &\propto F_h \left(n_s \cdot h\right)^{\alpha} \max\left(n_s \cdot \omega_o, 0\right) \\
...
Not only is Blinn-Phong more mathematically well-behaved, it has been shown that it fits significantly better to real BRDF data. See the article by \href{http://people.csail.mit.edu/addy/research/brdf/index.html}{Ngan et al.} for more detail.

The Blinn-Phong BRDF can be normalized using the similar way we did for the Phong BRDF. The closed-form is more complicated, so we just post the end result here and refer you to the article by \href{http://www.farbrausch.de/~fg/stuff/phong.pdf}{Fabian Giesen} for the derivation:
\begin{equation}
\rho_{\text{blinn}} = \frac{\left(\alpha + 2\right)\left(\alpha + 4\right)}{8\pi \left(2^{-\frac{\alpha}{2}} + \alpha\right)} F_h \left(n_s \cdot h\right)^{\alpha} \max\left(n_s \cdot \omega_o, 0\right).
\end{equation}
...
Step 1 is similar to Equation~\eqref{eq:phong_theta} and~\eqref{eq:phong_phi}, but instead of transforming the coordinates to the a basis formed by $r$, we use the basis formed by $n_s$. Generating an outgoing direction from Step 2 is then straightforward. However, the reflection in Step 2 is yet another change of variable that transform the PDF in the half vector space to the outgoing direction space. This time the Jacobian is not identity. We need to account for the derivatives between $\omega_o$ and $h$:
\begin{equation}
